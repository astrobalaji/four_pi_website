...
class AmaOnboarding(forms.ModelForm):
    ...
    tz = forms.FloatField(label = 'Time zone +/- hours in decimals from UTC', help_text = 'UTC: Coordinated Universal Time' , required = True, min_value = -12., max_value = 12., widget=forms.NumberInput(attrs={'style': 'width: 30%', 'cols': 200,'rows': 1}))
    ...
    telescope_flength = forms.FloatField(label = 'Telescope Effective Focal Length (mm)', required = True, min_value = 0., widget=forms.NumberInput(attrs={'style': 'width: 30%', 'cols': 200,'rows': 1}))
